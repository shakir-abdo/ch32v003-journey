...
تخيّل عدّاداً تنازلياً يدور ببطء. كل فترة، يجب على البرنامج **إعادة تصفيره**. إذا نسي البرنامج (لأنه تعلّق في حلقة لا نهاية أو crash)، الـ Watchdog يصل إلى الصفر و **يُعيد تشغيل الشريحة**.
...
- يجب **تصفير العدّاد داخل نافذة زمنية محددة**:
  - ليس قبل وقت معيّن (`W`).
  - وليس بعد أن ينقص العدّاد إلى 0.
- يكشف **تسارع البرنامج** (إذا تمّ التصفير مبكراً).
...
> ⚠️ **لا تُصفّر العدّاد داخل ISR**. إذا كان الـ main حلقة عالقة لكن ISR يُصفّر، الـ watchdog يفقد قيمته.
...
| العَرَض | السبب | الحل |
|---------|------|------|
| الـ MCU يعيد التشغيل عشوائياً | RLDR صغير + مهام طويلة | كبّر RLDR أو صفّر العدّاد أكثر |
...
3. **متى تُصفّر العدّاد؟**: إذا لديك 3 مهام بأوقات مختلفة، اختر تردد تصفير مناسب.
4. **WWDG window**: نفّذ النافذة الزمنية الضيقة.
5. **AWU + IWDG**: نام بعمق، استيقظ، صفّر العدّاد، نام مرة ثانية.
